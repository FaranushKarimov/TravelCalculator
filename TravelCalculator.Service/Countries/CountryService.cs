...
namespace TravelCalculator.Service.Countries
{
    public class CountryService : ICountryService
    {
        ...
        public async Task<List<Country>> GetCountriesFromContinet(int continentId)
        {
            return await _countryRepository.Entities.Include(x => x.Seasons).Where(x => x.ContinentId == continentId).Select(x => new Responce
            { Id = x.CountryId, CountryName = x.CountryName, IsPopularCountry = x.IsPopularCountry,Months = x.Season.Select(x => new MonthResponce {Month = x.Month.ToString(),SeasonType = x.SeasonType.ToString() }).ToList()}).ToListAsync();
        }
    }
}
